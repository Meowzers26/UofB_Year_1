...
int digit(int n) {    
    switch (isdigit(n)) {
        case 0:         // n is not a digit
            return -1;
            break;

        case 2048:      // n is a digit
            return 1;
            break;
    }
}
...
int convert(int n, const char mark[]) {
    ...
    if (nSize(n) == 1) {
        switch (n) {
            case 1:
                if (digit(mark[0]) == 1) {
                    convertedMark = mark[0] - '0';
                }
                else convertedMark = -1;
                break;

            case 2:
                if (digit(mark[0]) == 1 && digit(mark[1]) == 1) {
                    firstDig = mark[0] - '0';
                    secondDig = mark[1] - '0';
                    convertedMark = (firstDig * 10) + secondDig;
                }
                else convertedMark = -1;
                break;

            case 3:
                if (digit(mark[0]) == 1 && digit(mark[1] == 1) && digit(mark[2]) == 1) {
                    firstDig = mark[0] - '0';
                    secondDig = mark[1] - '0';
                    thirdDig = mark[2] - '0';
                    convertedMark = (firstDig * 100) + (secondDig * 10) + thirdDig;
                    
                    if (convertedMark != 100) {
                        convertedMark = -1;
                    }
                }
                else convertedMark = -1;
